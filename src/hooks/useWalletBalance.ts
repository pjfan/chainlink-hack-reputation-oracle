...
export const useWalletBalance = (address: string, chain: MoralisChainOptions): WalletBalance[] | null | undefined => {
  ...
  const getTokenPrice = async (inputToken: TokenBalance, chain: MoralisChainOptions): Promise<number | null> => {
    // slow down API call rate.
    await new Promise(resolve => setTimeout(resolve, 1200));
    console.log("Fetching price data for: " + inputToken.name);
    const price: number | null = await token.getTokenPrice({"address": inputToken.token_address, "chain": chain}).then((token) => token.usdPrice).catch((err) => {console.log(err); return null;});
    return price
  }
