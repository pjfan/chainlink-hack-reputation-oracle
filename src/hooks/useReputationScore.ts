...
export const useReputationScore = (
  ...
  const getAvgTxnsPastSixMonths = async (
    chain: MoralisChainOptions,
    address: string
  ): Promise<number> => {
    ...
    const pastSixMonthsTxns = await account.getTransactions({
      chain: chain,
      address: address,
      from_date: fromDate,
      to_date: toDate,
  ...
  const getAccountAgeMonths = async (
    chain: MoralisChainOptions,
    address: string
  ): Promise<number> => {
    // slow down API call rate.
      ...
      if (allTxns?.['total'] > allTxns?.['page_size']) {
        ...
        console.log(
          'Total transactions (' +
            ...
            allTxns?.['page_size'] +
            '). Getting last page using offset of: ' +
            offset
        );
    ...
    if (allTxns !== undefined && allTxns?.result !== undefined) {
      ...
      console.log(
        'Timestamp of first transaction: ' + moment(firstTxn['block_timestamp']).toString()
      );
      const monthsSinceFirstTxn: number = moment().diff(
        moment(firstTxn['block_timestamp']),
        'months'
      ); // off by 1
